...
internal static class RetrievalMetrics
{
        ...
        IReadOnlyDictionary<string, int> judgmentsByChunkId)
    {
        ...
        {
            var relevance = judgmentsByChunkId.GetValueOrDefault(rankedChunkIds[rank - 1], MinGrade);
            if (relevance >= 1)
            {
                return 1.0 / rank;
            }
        ...
        foreach (var (chunkId, grade) in judgmentsByChunkId)
        {
            if (grade < MinGrade || grade > MaxGrade)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(judgmentsByChunkId), grade, $"Judgment for chunk '{chunkId}' has grade {grade}, outside [{MinGrade}, {MaxGrade}].");
            }
        }
    }
}
